    ...
    <>
      ...
      <section className="pt-20 pb-12 border-b border-b-slate-500 h-[245px]">
        <h1 className="font-bold max-w-3xl mx-auto text-xl">
          Dia 19: La gente quiere hacer el mismo regalo pero a diferentes personas o en diferentes cantidades, agreguemos un botón de
          duplicar que abra el modal precargado.
        </h1>
      </section>
              ...
              {gifts?.map((gift) => (
                <div key={gift.id} className="flex justify-between items-center gap-1 h-[34px]">
                  <div className="flex items-center gap-2">
                    ...
                    {!isTyping && (
                      <p className="ml-2 text-xs" title={`Total: ${gift.price * gift.amount}`}>
                        (${gift.price} c/u - {gift.amount} u)
                      </p>
                    )}
                  </div>
                  {!isTyping && (
