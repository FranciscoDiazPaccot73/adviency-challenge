...
const Day19 = () => {
  ...
  const addGift = () => {
    if(duplicate){
      const duplicateGift = gifts.map((item)=>
      item.id === duplicate.id ? {...item, title:input, qty:count, addressee, image, mount} : item)
      setGifts([duplicateGift, ...gifts]);
      setTotal(total + duplicateGift.qty * duplicateGift*mount)
      setDuplicate(null)
    }
    ...
    if (editing) {
      ...
      if (gifts.find((item) => item.title === input) && duplicate === null) {
        ...
        const newGift = { id: Date.now(), title: input, addressee, qty: count, mount, image };
  ...
  return (
    <>
      <Head>
        <title>JOEL | Dia 19 | Adviency Challenge</title>
      ...
      <section className="pt-20 pb-12 border-b border-b-slate-500 h-[245px]">
        <h1 className="font-bold max-w-3xl mx-auto text-xl">
          Dia 19: La gente quiere hacer el mismo regalo pero a diferentes personas o en diferentes cantidades, agreguemos un botón de duplicar que abra el modal precargado.
        </h1>
      </section>
              ...
              gifts.map((gift) => {
                return <li className='flex items-center justify-between bg-gray-800/80 rounded-lg m-2 p-2'
                  ...
                  <div className='flex justify-end w-3/6'>
                    ...
                    <button className='text-xl mx-1 text-center bg-blue-600 px-1 rounded-lg' 
